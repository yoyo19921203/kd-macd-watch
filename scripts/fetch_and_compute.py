...
用法：
    ...
    python scripts/fetch_and_compute.py --backfill 90    # 回補最近 90 個日曆天歷史，第一次跑用一次即可
...
def build_report(target_date_str: str, lookback_cross_days: int = 5):
    ...
    for code, g in df.groupby("code"):
        ...
        out_rows.append({
            ...
            "kd3_strong": widening,
            "macd_positive": bool(macd.iloc[-1] > 0) if pd.notna(macd.iloc[-1]) else False,
            "rs5": rs(5), "rs20": rs(20),
        })
    out_rows.sort(key=lambda r: (not r["kd3_strong"], not r["macd_positive"], not r["vol_ok"]))
    return out_rows
